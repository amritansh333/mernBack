...
export const listDrawingRequests = async ({
  page = 1,
  limit = 10,
  search = "",
  status = "",
...
export const getDrawingRequest = async (id) =>
  DrawingRequest.findById(id).lean();
...
export const deleteDrawingRequest = async (id) => {
  ...
  const results = await Promise.allSettled(
    files.map((file) => deleteFile(file.relativePath)),
  );
  ...
  if (failed) {
    const error =
      failed.reason instanceof Error
        ? failed.reason
        : new Error("Failed to delete drawing files");
