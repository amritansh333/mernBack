...
export const updateMany = (ids, patch) =>
  Product.updateMany({ _id: { $in: ids } }, { $set: patch });

export const findEnquiriesByProductName = (productName, limit = 5) =>
  Enquiry.find({ product: { $regex: productName, $options: "i" } })
    .sort({ createdAt: -1 })
    .limit(limit)
    .lean();
